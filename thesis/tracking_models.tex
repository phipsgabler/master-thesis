...
references.  The graph construction is implemented in a function \jlinl{makegraph}.  The final
...
\jlinl{Graph} objects resulting from each of the models are shown in
...
outputs of the example models is shown in figure~\ref{fig:geom-deps}.\todo{add models in math
  notation?}
...
\enquote{trace dictionary} \jlinl{θ}, which contains all values of the random variables by name
...
this is out of scope of the present work.)  For example, \(p\) in the \jlinl{bernoulli_mixture}
...
factor.\footnote{This uses the interface of distribution objects from the
  \juliapackage{Distributions.jl} package, which have a \jlinl{support} method whose result is an
...
% For
% computational efficiency, a parallel Gibbs sampler \parencite{gonzalez2011parallel} could be
% constructed, improving scalability with increasing data size (i.e., numbers of observations).
Or, as in \juliapackage{Gen.jl}, a variant of \enquote{argument diffs} could be devised to prevent
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
and a truncated infinite mixture model (IMM) in stick-breaking construction, but otherwise of the
...
Lastly, two diagnostic values are plotted for each combination and chain.  The\todo{corner plots}
...
good convergence; as a rule of thumb, a value of more than \(1.1\) is suspicious\todo{add line in
  plots}.  The effective sample size (ESS) arises in the estimation of the asymptotic variance of
...
to the actual number of samples, or at least of the same order of magnitude.\todo{add line in plots}
