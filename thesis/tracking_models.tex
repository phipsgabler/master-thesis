...
the values very closely.  \todo{fix x-axis for extraction time plots; remove intercept}
...
samples would have zero autocorrelation except for the first lag.\todo{ACF on unthinned chains!}
...
number of samples, or at least of the same order of magnitude.\todo{publish measurement data}
...
\begin{figure}[p]
  ...
  \contcaption{Diagnostics, factored by algorithm, number of observations, and a selection of model
    parameters.  \(\widehat{\mathrm{R}}\) and ESS point estimates are jittered horizontally for better
    readability.  For chain plots and autocorrelation, the first chain of the respective combination
    has been used.}
\end{figure}
