...
centers, weights, and assignments \parencite[section 6.2]{marin2007bayesian}, a hidden Markov model
(HMM) with known variances and priors over transition and emission probabilities \parencite[section
7.3]{marin2007bayesian}, and an infinite mixture model (IMM) in stick-breaking construction, but
otherwise of the same form as the GMM, to represent a nonparametric example \parencite[section
2.2]{hjort2010bayesian}.  The models are implemented in \dppljl{} as shown in
listing~\ref{lst:evaluation-models}.
...
If follows a detailed analysis of the results per model in graphical form, based on six plots each.
First, PG and AG are compared in terms of sampling times by number of observations on the top left.
Right of this, we have the dependency of the extraction time of AG given the number observations.
The first of these points is almost always an outlier, since it involves additional compilation
time.  Through the rest, quadratic a quadratic function is fitted, which always matched the values
very closely.  \todo{fix x-axis for extraction time plots; remove intercept} On the facing page,
plots for convergence analysis are visualized.  On the top, the first chain of each combination is
shown as a qualitative representative (thinned for easier plotting).  Well-converging chains should
look like white noise, not like a random walk.  Below, the estimated autocorrelation function of the
same chains is given; these should decrease as quickly as possible to indicate good convergence
(uncorrelated variables would have zero autocorrelation except for zero lag).\todo{ACF on unthinned
  chains!}

Lastly, two diagnostic values are plotted for each combination and chain.  The
\enquote{scale-reduction} \(\hat{R}\), after \textcite{gelman1992inference}, estimates the factor by
which the scale of the current distribution might be reduced if the simulations were continued
\parencite[see][p. 285]{gelman2020bayesian}.  It should be close to \(1\) for indicating good
convergence; as a rule of thumb, a value of more than \(1.1\) is suspicious.  The effective sample
size (ESS) arises in the estimation of the asymptotic variance of Markov chains
\parencite[see][section 7.2]{vihola2020lectures}, where it is formally analogous to the sample size
in the variance estimator for \iid{} Gaussian variables.  The ESS can also be interpreted as a
...
\begin{figure}[t!]
  ...
  \caption{Sampling and extraction times for GMM.  Points jittered horizontally to increase
    readability.  A quadratic curve is fit to the extraction times.}
  \label{fig:plots-gmm}
\end{figure}

\subsection*{GMM}

The Gaussian mixture model was of the following form:
\begin{equation}
  \label{eq:gmm}
  \begin{aligned}
...
\begin{figure}[p]
  ...
  \contcaption{Extraction and sampling times.}
\end{figure}
