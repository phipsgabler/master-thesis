...
log-densities is independent from the normalization step, though, this can serve as a stating point
...
be static in a specific sense: the whole Markov blanket of the variable in question must be
reachedable and unique within one run of model tracking.  A large fraction of the models used in
practise do fulfill this condition, though.  As this problem is difficult so solve in general, the
same constraint applies to JAGS and BUGS, which makes \autogibbsjl{} not more limited than these.
...
So, overall, while the implementation in current form is not very applicable in practise, it is very
much so in principle.  Based on the lessons learned through this work, further contributions to
Gibbs sampling in \turingjl{} are already planned (although not necessarily based on
\autogibbsjl{}).
...
over cluster centers, weights, and assignments \parencite[section
6.2]{marin2007bayesian},
\begin{equation}
  \label{eq:gmm}
  \begin{aligned}
...
@model function hmm(x, K, ::Type{T}=Float64) where {T<:Real}
    N = length(x)
    s = zeros(Int, N)  # State sequence
    m = Vector{T}(undef, K)  # Emission matrix
    T = Vector{Vector{T}}(undef, K)  # Transition matrix

    # Assign distributions to each element of the transition matrix 
...
up such that each model is evaluated for ten chains per parameter combination, varying between AG
and PG, three values for the number of particles used by PG (since more particles usually lead to
better results, but higher sampling times), and three values for the number of observations (since
these determine the size of the trace, and thus have an influence on both AG's compile times and the
overall sampling time).  Concrete parameters in table~\ref{tab:autogibbs-params}.\todo{more
  meaningful table}
...
\begin{table}[t]
  ...
  \begin{tabularx}{\textwidth}{XXrrr@{\hskip 10mm}rrrr}
    ...
    \textbf{20 particles} & Chains & 10 & 10 & 10 & 10 & 10 & 10 & 10 \\
    \addlinespace
    \textbf{PG + HMC,} & Observations & 10 & 25 & 50 & 10 & 25 & 50 & 10 \\
    \textbf{35 particles} & Chains & 10 & 10 & 10 & 10 & 10 & 10 & 10 \\
    \addlinespace
    \textbf{PG + HMC,} & Observations & 10 & 25 & 50 & 10 & 25 & 50 & 10 \\
