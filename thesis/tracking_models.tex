...
ESS numbers are way too low.  A look at the exemplary autocorrelation plots confirms bad
...
Also for HMM, the same trends in sampling and extraction times is visible, with AG being consistenly
faster.  The extraction times seem to be quite the same as GMM, even in absolute terms.
...
being faster than AG on \(50\) observations (although that still does not scale, of course).

In this case, PG appears to work better on average.  In the chain plots, we can only see a
noticeable difference for \(\mu\), while the autocorrelation graphs are almost all worse for AG
...
observations, the increase seems to be much lower for AutoGibbs.  The growth of these curves appears
to be superlinear, perhaps quadratic.  For GMM and HMM, the maximal sampling time of AG is always
below the minimal sampling time of PG; and even in the case of IMM, AG's sampling time with \(50\)
observations is closest to PG's with only \(10\) particles.
...
buttressed by the distribution of the diagnostic values.  IMM poses a particularly bad application
for AG, but otherwise, no consistent \enquote{winner} is visible, and variations do not seem to
