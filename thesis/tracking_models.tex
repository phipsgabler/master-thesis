...
examine the two simple models shown in listing~\ref{lst:dependency-examples}\todo{fix caption
  alignment}.  The pretty-printed \jlinl{Graph} objects resulting from each of the models are shown
...
of the \enquote{trace dictionary} \jlinl{θ}, which contains all values of the random variables by
name (which is to view the probabilistic model as a joint density over trace dictionaries).  This is
done in the following simple fashion:
\begin{itemize}
  \firmlist
