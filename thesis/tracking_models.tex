...
⟨6⟩ = p ~ ⟨5⟩              ⤳ logpdf(DNP([0.3, 0.7], θ[w]), θ[p])
...
\begin{figure}[t]
  ...
  \caption{Association of the dependency graph of the example models from
    ...
    \texttt{DiscreteNonParametric} for better spacing.)}
...
\end{itemize}
All resulting expression are thereby to be understood as an implicit function of \jlinl{θ}.  These
...
adding the likelihood functions for each tilde statement, we get the symbolic log-joint density as,
for example,
\begin{lstlisting}
...
\begin{equation}
  ...
  % p(\lambda, m, x) = \distr{Gamma}(\lambda \given 2, 1/3)
  % \:\distr{Normal}(m \given 0, \, \sqrt{1 / \lambda})
  % \:\distr{Normal}(x \given m, \, \sqrt{1 / \lambda}).
\end{equation}
...
\begin{equation}
  \begin{aligned}
    ...
    % p(m \given \lambda, x) &\propto
    % \:\distr{Normal}(m \given 0, \, \sqrt{1 / \lambda})
    % \:\distr{Normal}(x \given m, \, \sqrt{1 / \lambda}), \\
    % p(\lambda \given m, x) &\propto \distr{Gamma}(\lambda \given 2, 1/3)
    % \:\distr{Normal}(m \given 0, \, \sqrt{1 / \lambda})
    % \:\distr{Normal}(x \given m, \, \sqrt{1 / \lambda}),
  \end{aligned}
\end{equation}
which in more technical terms are given through the \emph{Markov blanket} of \(m\) and \(\lambda\)\todo{reference}.



The complicated part of this step is the correction subsumption of indexed variables in the trace
dictionary: making sure that both \jlinl{θ[x][1]} and \jlinl{θ[x[1]]} are resolved correctly to the
same value.
...
the slow-down could be mitigated by improving key parts of \autogibbsjl{} and \irtrackerjl{}.
