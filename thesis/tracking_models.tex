...
outputs of the example models is shown in figure~\ref{fig:geom-deps}\todo{update \(p\) to \(\pi\)}.
...
supported variable~-- writing \(\pi\) instead of the parameter \(p\) to avoid confusing notation, we
get
\begin{equation}
...
Concretely, this works as follows, finalizing step 2 of the above scheme, exemplified by
...
\enquote{kernels}: closures returning distribution objects.  These closures can then be used to
construct a Gibbs-conditional sampler for usage in \turingjl{}'s \jlinl{Gibbs} sampler, in
combination with other samplers for the continuous variables.

Improvements: dynamic variant; extension for more kinds of variables (autoconj); parallel sampling
to scale on data; reuse of conditionals for trace-equivalent vectorized models.\todo{write
  improvements}
