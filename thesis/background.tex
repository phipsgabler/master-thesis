...
On top of that, the language is built on a very different, open compilation model.  Underlying the
...
into a sequence of labelled code blocks with (possibly conditional) jumps between them. Our example
...
selected, and type inference on its body be started.  If we go on and call \jlinl{foo([1.0])}, with
\jlinl{Vector{Int}} as the sole argument type, the following will be inferred:
% code_warntype
\begin{lstlisting}
