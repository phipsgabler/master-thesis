...
\textcite{griewank2008evaluating}, the standard work on the topic, and the survey by
...
\begin{equation}
  \begin{aligned}
    \Dif f(x, y) &= \Dif(g \circ (\sin \times \ident))(x, y) \\
    &= \Dif g\left( (\sin \times \ident)(x, y) \right) \circ \Dif(\sin \times \ident)(x, y) \\
    &= \left( (\Delta_1, \Delta_2) \mapsto \Delta_1 - \Delta_2 \right) \circ \left( \Dif\sin(x)
      \times \Dif \ident(y) \right) \\
...
\(\Dif \phi(x)\) by calculating the sensitivities to unit input perturbations \(\Delta_i = 1\) with
...
perturbation (\(\Delta\)), it maps a dual vector of an output perturbation (\(\mathfrak{d}\))
...
\(J{\phi}(x) \, \mathfrak{d}\), or equivalently the total derivative, for a fixed perturbation,
avoiding full matrix multiplication.  With \(\transpose{\delta}\) as the covector representation of
\(\mathfrak{d}\), we have
\begin{equation}
  \begin{aligned}
    \mathfrak{d} (\Dif \phi(x)(\Delta)) &= \transpose{\delta} J_{\phi}(x) \Delta \\
    &= \transpose{\left( \transpose{J_{\phi}(x)} \delta \right)} \Delta \\
    &= \CoDif \phi(x)(\mathfrak{d})(\Delta).
  \end{aligned}
\end{equation}
Since the unapplied \(\CoDif \phi(x)(\mathfrak{d})\) is itself an object in the dual space, it is
represented as a transposed matrix~-- and in fact, it is nothing else than a multiple of the
transposed Jacobian.  Recovering the gradient then reduces to evaluating it at a constant output
perturbation of \(1\), which is equivalent to the application of the primal differential to the
matrix of basis vectors~-- the identity matrix.
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
For our example function \(f\), this gives the same structural form of the result~-- only that now,
the value is a vector:
\begin{equation}
  \begin{aligned}
    \CoDif f(x, y) &= \CoDif (g \circ (\sin \times \ident))(x, y) \\
    &= \CoDif(\sin \times \ident) \circ \CoDif g((\sin \times \ident)(x, y)) \\
    &= (\CoDif\sin \times \CoDif\ident) \circ (\delta \mapsto [\delta, -\delta]) \\
    &= (\partial_1 \sin(x) \times 1) \circ (\delta \mapsto [\delta, -\delta]) \\
    &= \delta \mapsto [\delta \, \partial_1\sin(x), \delta \, 1] \\
    &= \delta \mapsto [\delta \, \cos(x), -\delta]
  \end{aligned}
\end{equation}
In this form, starting with a perturbation \(\mathfrak{d}(\cdot) = 1\) (a skalar in the matrix
version), we get back the gradient tuple through just one evaluation!
