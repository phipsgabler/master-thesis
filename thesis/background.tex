...
structures should exist to allow complex behavior, such as to\todo{page break}
\begin{itemize}
  \firmlist
...
\end{lstlisting}
The result is then spliced back into the AST, which is compiled further as if it were written as\todo{pagebreak}
\begin{lstlisting}
function foo(x)
    y = zero(eltype(x))
    for i = eachindex(x)
