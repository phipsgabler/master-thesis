...
For multi-component structures, where the latent variables can be subdivided into several
...
In many real-world models, the factorization structure of the underlying graph is quite sparse and
...
\end{lstlisting}
A model instance \jlinl{model} is created first.  The value of \jlinl{forward} will then be a
...
\jlinl{x_observations}, created using the Metropolis-Hastings algorithm.  If we were to write out
...
of sufficient syntactic flexibility, for example \juliapackage{Gen.jl}
...
different values for the sampling algorithm, which in turn will use the inner \jlinl{evaluator}
function of the instance to run the model with chosen \jlinl{sampler} and \jlinl{context} arguments,
that are passed to the \enquote{tilde functions}, to which the statements of the form \jlinl{expr ~
  D} are converted.
...
functions for the variables \jlinl{x[i]} in listing~\ref{lst:normal-expanded} are distinguished in a
...
\jlinl{sample} interface, the resulting values of sequential evaluations are then stored in a
\jlinl{Chains} object, a data frame containing a value for each variable at each sampling step.
...
The key element to Julia's versatility is its multiple dispatch approach.  The terminology of Julia
uses \emph{function} only for the name of certain callable objects, which can have multiple
\emph{methods} for different combinations of argument types.  When a function is applied, the types
of the arguments are determined, and the most specific matching methods selected and called.  For
example, the \protect\jlinl{+} function has many methods for different types, defined at various
places, and can be freely extended by users for custom types.  This flexibility eliminates the need
for many organizational mechanisms such as interfaces or inheritance, without compromising
expressibility.
...
simple text-substitutions as C preprocessor macros.  As an example, look at the following method
that sums up the \jlinl{sin} values of a list of numbers:
\begin{lstlisting}
function foo(x)
    y = zero(eltype(x))
...
After macro expansion, the code of the method is \emph{lowered} into an intermediate representation
...
array accesses, \jlinl{x[i]}, are replaced by calls to the library function \jlinl{getindex(x, i)}.
The for loop in the example is converted into the equivalent of a while loop using the
\jlinl{iterate} library function:
\begin{lstlisting}
...
So, in a sense, JIT compilation can be seen as a process that returns compiled code, given a
...
While in some cases the loss function is simple enough to find the gradient by manual calculation,
...
The appendix~\ref{sec:ad-details} contains a more formal introduction of the underlying concepts.
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
(Again, see appendix~\ref{sec:ad-details} for mathematical details).  As in building up reverse
computation graphs, the adjoint code will invert the control flow of the basic blocks in the primal
function, taking into account that data flow may involve dynamic dependencies.  In our little
example (without control flow), this leads to
\begin{equation}
