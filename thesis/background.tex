...
sldk sldks dlks dlksfj dlkfjs lkdjf slkdjls dkf\texttt{\S}
...
The macro invocation of \jlinl{@show} will be converted to a function call receiving as input the
expression
\begin{lstlisting}
...
taken as is and used to print the value of the expression, labelled by it's form in the code:
\begin{lstlisting}
function foo(x)
    y = zero(eltype(x))
    for i = eachindex(x)
...
representation consisting of only function calls and branches.  For example, the for loop in the
example is converted into code equivalent to
\begin{lstlisting}
...
15 ─       goto §16
16 ─       goto §17
17 ─       invoke Base.println("y += sin(x[i]) = "::String, %32::String)::Any
│    %37 = (%15 === %3)::Bool
└───       goto §19 if not %37
18 ─       goto §20
