...
methods for differentiation have been
developed\parencite{griewank2008evaluating,baydin2018automatic}.  These can be categorized into
three classes:
\begin{itemize}
  \firmlist
...
graph at ov the evaluation at a point, which is what makes the topic relevant for this thesis.

To understand how this works, let us first start with the mathematics.  What is a derivative,
really?  When we talk about gradients, which is what we really need in a gradient algorithm, this is
usually a rather informal term for \enquote{the vector of partial derivatives}, which then points
into an ascent direction.  This is however not the most natural form to work with in a compositional
approach.  Instead of starting with a limit of tangents, more insight is provided by viewing
...
writing \(\Dif f(x) = A\).  When the derivative exists for all \(x\), we can use \(\Dif\) as a
well-defined higher-order function on its own; we will assume this in the following.

The important fact here is that \(\Dif f(x)\) is still a function: specifically, a linear function
approximating how \(f\) reacts to a perturbation, \(\Delta\), around \(x\).  Or, in other words:
\begin{equation}
  f(x + h) = f(x) + Df(x)(h) + o\left(\enVert{h}\right).
\end{equation}
...
but also \(\psi\) \parencite{elliott2018simple}.  This is not as bad as it seems, though: as we see
below, automatic differentiation algorithms evaluate both \(((\phi \circ \psi))(x)\) and
...
\newthought{To construct} the full gradient from this, however, requires to evaluate \(\Dif f(x)\) at
...
If \(f: U \subseteq \RR^N \to \RR\), as is the case for loss functions of parametric models, the
...
\begin{figure}[t]
  \subbottom[Forward mode.]{\includegraphics[width=0.45\textwidth]{figures/comp-graph}}
  \hfill
  \subbottom[Backward mode.]{\includegraphics[width=0.45\textwidth]{figures/comp-graph-backward}}
