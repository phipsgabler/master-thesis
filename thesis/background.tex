...
There is one noteable complication regarding conversion to SSA form: we need to be able to
...
\newthought{A key principle} in Julia's compilation model is type specialization.  As we have seen,
whenever a function call is evaluated, the types of the arguments are first determined, and then the
most specific method selected.  This automatically gives the language dynamic semantics: an
implementation could perform this operation simply at every call.  In practice, however, at this
point multiple dispatch and JIT compilation combine into one of the main principles of optimization.
Instead of evaluating the same code over and over again, methods are JIT-compiled the first time
they are called.  The compiled code is then cached in the method table.  Method compilation does not
happen recursively at once, though.  Only when the body of a compiled method is then executed with
concrete arguments, the same process is performed again, for each invoked method.

So, in a sense, JIT compilation can be seen as a function that returns compiled code, given a
function and a tuple of types.  Similarly to macros, transform original code, given an expression,
this process of generating compiled methods from types is customizable in Julia: via so-called
\emph{generated functions}, there exists a process to dynamically generate code, given argument
types~-- a form of stated programming \parencite{rompf2010lightweight,bolewski2015staged}. Such
generated functions, when called, instead of being directly translated into machine code, emit new
code to the compiler, which is then JIT-compiled.  For example, when we have two methods of a
function \jlinl{f}:
\begin{lstlisting}
f(x::Int) = println("Int")
...
\end{lstlisting}
Calling \jlinl{f_generated(1)} will then determine the argument type (\jlinl{Int}), and pass it to
the function body of \jlinl{f_generated}.  There, the conditional will dispatch on the first branch,
and the expression value \jlinl{:(println("Int"))} be returned.  This is now passed back to the
...
code of \jlinl{:(println("Int"))} directly looked up\footnote{Technically, the compiler is always
  free to call the generating code multiple times~-- which is the reason generated functions should
  never involve side effects or depend on external state.}.  Of course, simply replacing dispatch,
as with this example, is not what generated functions are used in practise.  Must usages concern
types with statically known shape arguments, such as tuples, named tuples, or array ranks.  They can
also be used for type-level computation of values that become known only known at runtime, through
singleton types such as \jlinl{Val}.

The direct generation of code, given argument types, is however not the furthest we can go.  First,
generated functions are not only allowed to return either \jlinl{Expr} objects~-- the internal
representation of the surface AST~-- but also \jlinl{CodeInfo} objects, which are the lowered
internal representation in (almost) SSA form.  This, on its own, would not be of much use most
times, but there is a second, more interesting feature: it is possible to query the \jlinl{CodeInfo}
of a method by reflection.  Combining these two, we now have all the tools to implement IR-level
code transformations as follows:
\begin{enumerate}
  \firmlist
\item Define a generated function, that takes as arguments another function and its arguments
\item Within this function, obtain the IR of the passed function.
\item Transform this IR however we like, changing the inner function calls to be called with the
  same transformation
\item Return the IR.  It will now get called with the same arguments.
\end{enumerate}
Importantly, unlike macros, such transformation can be performed \emph{recursively}: by applying the
same transformation to the inner function calls.  Also, since they operate not during parsing, the
function to be transformed needs not be known at compile time, and not be present literally in the
code~-- the generated function can be called on every available callable object, at any time during
runtime.  This makes it possible to transform even functions from other libraries.  One particular
example of this is automatic differentiation, as shown in the next chapter: a call to
\jlinl{gradient(f, x, y)} will obtain the code for \jlinl{f}, produce differentiated code, and call
the result on \jlinl{x} and \jlinl{y}; naturally, differentiating \jlinl{f} involves recursively
differentiating the other, unknown functions within it, too, and combining the results using the
chain rule.
...
code of the called function, it is possible to change what \enquote{invoking a method} within its
body means!  Through this, abstract interpretation patterns can be realized, by extending the
existing data path with additional metadata (e.g., AD, or concolic execution), or non-standard
execution be implemented (e.g., CPS transformations).  There exist already two Julia packages trying
to simplify working with this kind of transformation: \juliapackage{Cassette.jl}, which provides
overloadable function application by a so-called \enquote{overdubbing} mechanism; and
\juliapackage{IRTools.jl}, which has a more user-friendly alternative to \jlinl{CodeInfo}, and a
macro similar to \jlinl{@generated} that makes writing recursive functiona IR-transformation using
this data structure easier.
...
how the necesary transformations arise in practise.\todo{WTF.  Differentiate again.}

What is a derivative, really?  Instead of starting with a limit of tangents, more insight is
provided by viewing it as a best-approximating linear operator.  One of the most generaly useful
definitions is provided through the \emph{Fréchet derivative}.  A bounded linear operator
\(A: V \to W\) the derivative of a function \(f: U \subseteq V \to W\) at \(x\), if
\begin{equation}
  \label{eq:frechet}
  \lim_{\enVert{h}_V \to 0} \frac{\enVert{f(x + h) - f(x) - A(h)}_W}{\enVert{h}_V}
\end{equation}
In other words,
\begin{equation}
...
writing \(Df(x) = A\).  This essentially generalizes the notion of the \emph{total differential}.
When the differential exists for all \(x\), we can use \(D\) as a well-defined higher-order function
on its own, as in the following.

(writing \(g = (x, y) \mapsto x - y\) instead of the infix subtraction operator) Chain rule:
\(D(\phi \circ \psi)(x) = (D(\phi(x)) \circ D\psi(x))\)
\begin{equation}
  \begin{aligned}
    Df(x, y)(\Delta) &= D(g \circ (\sin \otimes \operatorname{id}))(x, y)(\Delta) \\
    &= (Dg(\sin \otimes \operatorname{id})(x, y) \circ (D\sin \otimes D\operatorname{id})(x, y))(\Delta) \\
    &= (D\sin(x) - D \operatorname{id}(y))([\partial\sin(x)(\Delta_{1}), \Delta_{2}]) \\
    &= \partial \sin(x) \Delta_{1} - 1 \Delta_{2} \\
    &= \cos(x) \Delta_{1} - \Delta_{2}
  \end{aligned}
\end{equation}

In particular, if \(f: U \subseteq \RR^N \to \RR\), as is the case for loss functions, the partial
derivatives can be recovered by applying the Fréchet derivative to basis vectors \(e^i\):
\begin{equation}
  Df(x)({e}^i) = \partial_i f(x)
\end{equation}
To recover the gradient from this, however, requires to evaluate \(Df(x)\) at all the \(N\) basis
functions.  In order to overcome this, we can rewrite the above equation as
\begin{equation}
  \begin{aligned}
    \partial_i f(x) &= Df(x)({e}^i) \\
    &= \langle 1, Df(x)({e}^i) \rangle \\
    &= \langle D^{*}f(x)(1), {e}^i \rangle,
  \end{aligned}
\end{equation}
where \(D^{*}f(x)\) is the \emph{adjoint operator}.
...
\begin{equation}
  \begin{aligned}
    Df(x)(\mathrm{e}^i) &= \left\langle 1, \transpose{[\cos(x), -1]} {e}^i \right\rangle \\
    &= \left\langle [\cos(x), -1], {e}^i \right\rangle
  \end{aligned}
\end{equation}

(Note that the adjoint operator inverses the order of composition)
\begin{equation}
  \begin{aligned}
    D^{*}f(x, y)(\delta) &= D^{*}(g \circ \sin \otimes \operatorname{id})(x, y)(\delta)\\
    &= (D^{*}\sin \otimes D^{*} \operatorname{id} \circ D^{*}g)(x, y)(\delta) \\
    &= ((D^{*}\sin(x) \otimes D^{*} \operatorname{id}(y)) \circ (\xi \mapsto [\xi, -\xi])) (\delta) \\
...
\begin{figure}[t]
  \centering
  \subbottom[Forward mode.]{\includegraphics[width=0.49\textwidth]{figures/comp-graph}}
  \bigskip
  \subbottom[Backward mode.]{\includegraphics[width=0.49\textwidth]{figures/comp-graph-backward}}
  \caption{Computation graph and intermediate expressions of the expression \protect\jlinl{sin(x) -
      y}, together with the derivative graphs in forward- and backward mode.}
