...
are exactly multiplications by a scalar.  Therefore, we can recover
\begin{equation}
  \begin{aligned}
...
\begin{figure}[t]
  \subbottom[Forward mode.\label{fig:comp-graph-fw}]{%
    \includegraphics[width=0.45\textwidth]{figures/comp-graph}}
  \hfill
  \subbottom[Backward mode.\label{fig:comp-graph-bw}]{%
    \includegraphics[width=0.45\textwidth]{figures/comp-graph-backward}}
...
\end{figure}
\todo{convert omega to noto, absolute font size}
...
matrix multiplication~-- but the result already a scalar, which is the reason we have to apply it to
the basis vectors to get back the gradient.  Backward mode, on the other hand, calculates the
product of the Jacobian with the operator that should be applied to the result, and returns a matrix:
\begin{equation}
  \begin{aligned}
  ...
  \end{aligned}
\end{equation}
Where we assume \(\mathfrak{d} = \Omega \mapsto \transpose{d}\Omega\).  Since the unapplied
\(\CoDif \phi(x)(\mathfrak{d})\) is itself an object in the dual space, it is represented as a
transposed vector~-- and in fact, it is nothing else than a multiple of the transposed Jacobian.
Recovering the gradient then reduces to evaluating it at a constant output perturbation of \(1\),
which is equivalent to the application of the primal differential to the matrix of basis vectors.
...
the value is a vector.  \(\mathfrak{d}\) can in this case be identified with a scalar \(\delta\),
therefore:
\begin{equation}
  \begin{aligned}
...
\enquote{trick} \parencite{bishop2006pattern}!
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
which is displayed in the red graph in \ref{fig:comp-graph-bw}.
...
shape of \(\RR^N \to \RR^N\), so forward mode is usually faster.  In general, unfortunately,
determining the optimal order of derivative evaluation is hard~-- this so-called \emph{optimal
  Jacobian accumulation} problem is known to be NP-complete \parencite{naumann2007optimal}.
...
log-densities in a probabilistic model, and large numeric simulations).

The second possibility is \emph{operator overloading}.  Forward mode can be recast in mathematically
equivalent form by using dual numbers \parencite[][section 3.1.1]{baydin2018automatic}.  These
...
\begin{equation}
  (z, \dot{z}) = (\phi(x), \Dif\phi(x)(\dot{x})) \Leftrightarrow z + \dot{z}\epsilon = \phi(x + \dot{x}\epsilon).
\end{equation}
(generalization to higher dimensions, as well as higher derivatives in form of hyper-dual numbers,
follow equally natually.)
...
This is exactly what graph-based machine learning frameworks do: PyTorch
\parencite{paszke2017automatic}, TensorFlow \parencite{abadi2015tensorflow}\footnote{Strictly
  speaking, TensorFlow (unless in eager mode) always builds up a complete expression graph, which is
  differentiated symbolically, and is therefore somewhat in the middle between operator overloading
  (since the graph is a runtime data structure) and source transformation (since the graph undergoes
  various \enquote{static} transformations, and is then \enquote{lowered}).}, DyNet
\parencite{neubig2017dynet}, and Chainer \parencite{tokui2015chainer}.
