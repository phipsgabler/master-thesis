...
\end{lstlisting}
    \caption{Macro-expanded and slightly simplified code of the model in
    listing~\ref{lst:normal}.\label{lst:normal-expanded}}
\end{lstfloat}
...
\end{lstlisting}
The macro invocation of \jlinl{@show} will be treated by the compiler, during parsing, as a function
call receiving as input the following data structure, representing \jlinl{y += sin(x[i])} in
...
converted into a sequence of labelled code blocks with (possibly conditional) jumps between
them. Our example is therefore lowered into four blocks as in listing~\ref{lst:foo-inferred}.  Note
that until now, the operations involved were purely syntactic in nature, and could be perfomed by
solely taking into account the code of the function \jlinl{foo}.  As soon as \jlinl{foo} is called
on a concrete type, though, the most specific method fitting to the argument types will be selected,
and type inference on its body be started.  If we go on and call \jlinl{foo([1.0])}, with
\jlinl{Vector\{Int\}} as the sole argument type, the following will be inferred~-- the result of our
example as the annotations in the same listing.
\begin{lstfloat}
\begin{lstlisting}[style=lstfloat]
1 ─ %1  = eltype(x)::Compiler.Const(Float64, false)
│         (y = zero(%1))
...
4 ┄       return y
\end{lstlisting}
  \caption{Lowered code of the example function \texttt{foo} defined above, annotated with inferred
    types (as through \protect\jlinl{@code_warntype}).\label{lst:foo-inferred}}
\end{lstfloat}

Bla bla \parencite{singer2018static}.
...
└───       return %48
\end{lstlisting}
    \caption{Typed and optimized code in SSA form.}
\end{lstfloat}
