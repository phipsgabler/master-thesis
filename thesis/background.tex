...
mode~-- only that now, the value is a vector.  \(\mathfrak{d}\) can in this case be identified with
a scalar \(\delta\), therefore:
\begin{equation}
  \begin{aligned}
...
off\footnote{As a rule of thumb in Julia, for \(f: \RR^M \to \RR^N\), forward mode is usually faster
  when \(M \ll N\) or as long as \(M \lessapprox 100\).  This folklore should always be confirmed by
  benchmarking, though.  See
